...
const getStudentofBatch = async (req, res) => {
  try {
    const { batchId } = req.params;
    console.log("Fetching batch students for:", batchId);
    ...
    // Ensure students exist, otherwise return `0`
    if (!batchStudents.enrolledStudents || batchStudents.enrolledStudents.length === 0) {
      console.warn("No students enrolled in this batch.");
      return res.status(200).json({ message: "No students enrolled", students: 0 });
    }

    console.log("Batch Students Found:", batchStudents.enrolledStudents.length);
    res.status(200).json({ message: "Batch Students", students: batchStudents.enrolledStudents });
  } catch (error) {
