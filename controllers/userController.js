...
exports.updateTrials = async (req, res) => {
  ...
  try {
    ...
    const trialRequest = new TrialRequest({
      ...
      academyId: batch.academyId, // From batch
      batchId: batch._id,
      requestedAt: new Date(), // You can set the trial date as per requirement
      trialTime: "Not Set", // Update with the actual trial time if available
      status: "requested", // Set status to "requested"
    });
  
    await trialRequest.save(); // Save the trial request
