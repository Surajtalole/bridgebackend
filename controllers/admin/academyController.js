...
exports.verifyAcademy = async (req, res) => {
  try {
    ...
    const { verify } = req.body; // Get verify status from request
    ...
    academy.verify = verify; // Toggle verification status dynamically
    await academy.save();

    res.status(200).json({ message: `Academy ${verify ? 'verified' : 'unverified'} successfully` });
  } catch (error) {
